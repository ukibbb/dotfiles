...
<Warning>
 Background images are currently duplicated in VRAM per-terminal.
...
<Note>
 `global:` is only supported on macOS and certain Linux platforms.
</Note>
...
<Note>
 this is only supported on macOS. The GTK runtime does not support
...
<Note>
 When the macOS application is hidden, keyboard layout changes
