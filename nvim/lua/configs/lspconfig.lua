...
local function schedule_ts_restart(client_id, reason)
  if ts_restart_pending[client_id] then
    return
  end
  ...
  local function restart()
    ...
    if not client or client.name ~= "ts_ls" or client:is_stopped() then
      return
    end
...
local function ts_log_message_handler(err, params, ctx, config)
  ...
  if err or not params or type(params.message) ~= "string" then
    return result
  end
