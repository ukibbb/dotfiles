...
| `Cmd+j` | `Ctrl+Alt+j` | `<M-C-j>` | Telescope: next result |
| `Cmd+k` | `Ctrl+Alt+k` | `<M-C-k>` | Telescope: previous result |
...
These are sent directly by Ghostty when not remapped by Karabiner:

| You Press | Neovim Sees | Action |
|-----------|-------------|--------|
| `Cmd+n` | Escape sequence | (available for mapping) |
| `Cmd+j` | `<D-j>` | Telescope: next result fallback |
| `Cmd+k` | `<D-k>` | Telescope: previous result fallback |
...
| You Press | Neovim Key | Action |
|-----------|------------|--------|
| `Cmd+j` / `Alt+j` | `<M-C-j>` (`<D-j>` fallback) / `<M-j>` | Navigate to next result |
| `Cmd+k` / `Alt+k` | `<M-C-k>` (`<D-k>` fallback) / `<M-k>` | Navigate to previous result |
